...
LeptonGen::LeptonGen(float max_pt) :
    _max_pt(max_pt)
{
...
int LeptonGen::nLeptons() {
    return _lep_eff_dist(_rng);
}

float LeptonGen::pt() {
    return _pt_dist(_rng);
}

float LeptonGen::eta() {
    return _eta_dist(_rng);
}

float LeptonGen::phi() {
    return _phi_dist(_rng);
}

unsigned LeptonGen::flavor() {
    return _lep_eff_dist(_rng);
}
...
bool LeptonGen::isLoose() {
    return _trig_dist(_rng) > 0.2;
}

bool LeptonGen::isMedium() {
    return _trig_dist(_rng) > 0.5;
}

bool LeptonGen::isTight() {
    return _trig_dist(_rng) > 0.8;
}

JetGen::JetGen(float max_pt, int max_n) :
    _max_pt(max_pt),
    _max_n(max_n)
{
...
float JetGen::mass() {
    return _pt_dist(_rng) * 15.;
}

unsigned JetGen::truthHadronId() {
    return _jet_eff_dist(_rng);
}

unsigned JetGen::nTrk() {
    return _jet_ntrk_dist(_rng);
}

float JetGen::bTagScore() {
    return _trig_dist(_rng) * 100.;
}

MetGen::MetGen(float max_pt) :
    _max_pt(max_pt)
{
    _pt_dist = std::uniform_real_distribution<float>(0, _max_pt);
    _phi_dist = std::uniform_real_distribution<float>(-3.14, 3.14);
...
EventGenerator::EventGenerator(float max_pt, unsigned max_nlep, unsigned max_njet) :
    _max_pt(max_pt),
    _max_nleptons(max_nlep),
    _max_njets(max_njet)
{
